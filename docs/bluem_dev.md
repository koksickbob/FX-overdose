...
- PnL 숫자 카드는 208px, 색상 선 그래프 카드는 176px 기준으로 서로 폭을 교환했다.

## 2026-08-01 — 전체 스킨 검수용 임시 해금

- 새 게임 초기화 시 `CostumeManager` 카탈로그의 모든 코스튬을 보유 상태로 시작하도록 임시 검수 플래그를 활성화했다.
- 기본 장착 복장은 그대로 유지하며 상점 APPAREL 목록에서 원하는 스킨을 즉시 장착해 확인할 수 있다.
- `UnlockAllCostumesForReview` 플래그로 격리했으며 사용자가 롤백을 요청하면 기존 기본 복장 단독 보유 초기화로 복원한다.
- 업적 전체 해금용 임시 분기는 검수 후 제거했으며, 다시 실제 PlayerPrefs 달성 기록과 정상 진행률만 표시한다.

관련 파일:

- `Assets/Scripts/Items/CostumeManager.cs`

## 2026-08-01 — 한복·유카타 표시 크기 보정

- 한복·유카타 각 35장, 총 70장의 `600×1180` 원본에서 알파값 8 초과 픽셀의 실루엣 바운드를 직접 측정했다.
- 동일 포즈의 기본 요미 35장을 직접 기준으로 이미지별 균일 배율을 계산했다.
- 한복은 `1.05094~1.33493배`, 유카타는 `1.01921~1.37654배` 범위의 70개 개별 실측값을 적용한다. 기존 `1.10배` 일괄 보정과 타 코스튬 중앙값 기준은 제거했다.
- 크기 보정 후 각 이미지의 실루엣 좌우 중심과 발끝 좌표도 기본 요미의 동일 포즈와 맞춰 70개별 X/Y 오프셋을 적용했다.
- 최외곽 알파 높이를 그대로 맞출 때 의상 실루엣이 체감상 커 보이는 점을 반영해 한복·유카타에 `0.95` 광학 배율을 적용하고, 중앙 피벗 축소로 발끝이 올라가지 않도록 Y 위치를 함께 보상했다.
- 감정, 포지션, 스킬 강화, 아이템 사용, 오버도즈 스프라이트가 전환될 때 해당 이미지의 배율을 즉시 적용하며, 다른 코스튬과 누락 시 기본 스프라이트는 `1.0배`를 유지한다.
...
- UI 확인 중 임시로 1일차에 배치했던 편의점 사장 보스를 원래 일정인 3일차로 복구했다.
...
- 보스 스프라이트 없이도 사용할 수 있는 전광판형 등장 연출 초안을 추가했다.
- 보스 생성 시 화면 암전, `BOSS DETECTED` 경고선, 보스명·설명과 `VS` 배지가 슬라이드 인되고 약 2초 뒤 퇴장한 다음 기존 실시간 HUD로 이어진다.
- 등장 연출 확인 후 편의점 사장의 출현일을 원래 일정인 3일차로 복구했다.
...
## 2026-07-30 — 모니터 비율 대응 UI 스케일

- 가로형 루트 캔버스의 기준 해상도를 1920×1080으로 통일하고 화면 비율에 따라 스케일 기준을 동적으로 선택한다.
- 16:9보다 넓은 화면은 높이 기준, 더 좁은 화면은 너비 기준으로 맞춰 HUD 크기와 화면 점유율이 해상도마다 크게 달라지지 않게 했다.
- 창 모드에서 해상도를 변경하면 즉시 다시 계산하며, 런타임에 뒤늦게 생성되는 업적·로딩 UI에도 자동 적용한다.
- 월드 공간 UI와 세로 구도로 제작된 컷신 캔버스는 기존 연출 비율을 보존하기 위해 적용 대상에서 제외한다.
