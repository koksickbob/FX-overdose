...
| **T4** | 91+ | 전면화 | 세계가 오빠 하나로 축소된다. 시선 고정, 거리 제로. **단 수위는 애원·응석까지 — 공포 연출 금지** |

> 현행 코드의 대화 토픽 해금 경계(0/31/61, `YomiTalkTopics.MinAffection`)와 T1~T3 경계가 일치한다. T4(91+)는 이번에 신설되는 연출 전용 구간이며 토픽 해금과는 무관하다.
> 구간 경계의 단일 진실 원천은 [Affection_Tier_Table.md](../P2_04_System/Affection_Tier_Table.md)다 (2026-08-14 확정 — 90은 T3다).
