...
| 멤버 변수명 | 타입 | 설명 및 역할 |
| :--- | :--- | :--- |
| `OptionType` | `ChoiceOptionType` (Enum) | 선택지 유형 (`Safe`, `Aggressive`, `SpecialItem`) |
| `OptionTitle` | `string` | 선택지 버튼에 표시될 요약 문구 (예: `"[관망 제어] 진입 차단 및 관망"`) |
...
* **상세 연동 순서**:
  ...
  3. **차트 강제 오버라이드 (`MarketSimulationEngine`)**:
...
- [ ] **1단계**: `ChoiceOptionData` 및 `ChoiceEventSO` C# 스크립트 작성 (직렬화 및 Enum 정의)
- [ ] **2단계**: 10종 돌발 이벤트 ScriptableObject 애셋(`EVENT_01` ~ `EVENT_10`) 유니티 Editor 생성 및 데이터 입력
- [ ] **3단계**: `ChoiceEventController` 코어 엔진 작성 (시간 트리거 및 효과 적용 분기 로직)
