...
| **하지 않을 것** | CG 이미지 생성 — 5절은 **제미나이 발주용**, [NovelAI 시트](P2_05_Prologue_Cutscene_NovelAI_Prompt_Sheet.md)는 **오너가 직접 생성**하며 둘 다 코덱스 몫이 아니다. 에셋이 없는 동안 단색 플레이스홀더(짙은 남색 등)로 배경 슬롯을 채우고 진행할 것 |
...
## 7. NovelAI 발주용 프롬프트 → **별도 문서로 분리** (2026-08-16)

**[P2_05_Prologue_Cutscene_NovelAI_Prompt_Sheet.md](P2_05_Prologue_Cutscene_NovelAI_Prompt_Sheet.md)**

NovelAI는 단부루 태그 모델이라 5절(제미나이용 산문)과 문법이 근본적으로 다르고, 네거티브도 UC 별도 필드로 가야 한다. 게다가 그쪽은 [`nai_generate.py`](../../nai_generate.py)가 **파싱하는 대상**이라 코드 블록 순서가 계약이 된다 — 이 문서에 섞어두면 5절을 손보다가 스크립트를 깨뜨린다. 그래서 뺐다.

- **5절(제미나이용)은 그대로 유효하다.** 대체 관계가 아니라 발주처가 둘인 것이다.
- 이 문서가 계속 원본이다 — 스토리 노드 분해(3절)·컷 리스트(4절)·검수 체크리스트(6절)는 NovelAI 시트도 여기를 참조한다.
- **0.1절 "하지 않을 것"은 그대로다.** CG 생성은 코덱스 몫이 아니다.
