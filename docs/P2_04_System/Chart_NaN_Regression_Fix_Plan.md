...
| C7 기준 배속 | ✅ **씬을 `0.666`(하루 10분)으로 상향** — 아래 참조 |
...
### C7 — 기준 배속 0.666 상향 (하루 현실 10분)

당초 `0.666f`를 "씬이 덮어쓰는 죽은 값"으로 봤으나 [GameManager.cs:92](../../Assets/Scripts/GameManager.cs#L92)의 주석이
**"기존 1.0초에서 1.5배 가속 → 하루 현실 10분"**이라는 의도된 설계 변경임을 밝히고 있었습니다.
씬만 `1`(하루 15분)에 머물러 있어 의도가 반영되지 않은 상태였고, 기획 판단에 따라 씬을 코드 의도에 맞췄습니다.

| 씬 | 변경 | 비고 |
| --- | --- | --- |
| `GameScene.unity` | `1` → **`0.666`** | 936행 |
| `tutorial.unity` | `1` → **`0.666`** | 747행 |
| `SampleScene.unity` | 변경 없음 | 빌드 설정 `enabled: 0` — 미출하 |

하루 길이가 현실 **15분 → 10분**으로 줄어듭니다. `GameManager`의 코드 기본값은 이제 씬과 일치하므로 그대로 뒀습니다.

### 시간 배속을 세이브에서 복원하지 않도록 변경 (동반 필수 변경)

씬만 바꾸면 **기존 세이브에는 적용되지 않습니다.** slot_2에 `SecondsPerGameMinute = 1.0`이 기록돼 있어
복원 시 그 값이 다시 주입되기 때문입니다. 그래서 복원 자체를 끊었습니다.

근거는 이 값이 **플레이어 상태가 아니라 설계 상수**라는 점입니다. `GameManager.SetSecondsPerGameMinute`의 호출처는
[DynamicTimeRegulator.cs:58](../../Assets/Scripts/Core/DynamicTimeRegulator.cs#L58)의 슬로우 모션 lerp **하나뿐**이라,
세션 중 이 값이 씬 기준값에서 영구히 벗어나는 경로가 없습니다.

- [SaveLoadManager.cs](../../Assets/Scripts/System/SaveLoadManager.cs#L484) — 주입 블록 제거, 사유 주석으로 대체
- [SaveData.cs](../../Assets/Scripts/System/SaveData.cs#L46) — 필드는 **기록(진단) 전용**으로 남김. 불러오기 때 사용하지 않음

부수 효과로 3.3절의 `cameFromTradingSession` 조건이 불필요해져 함께 제거됐습니다.
기준 배속의 출처가 **씬 하나**로 정리되어, 앞으로 배속을 조정하면 기존 세이브에도 즉시 반영됩니다.
