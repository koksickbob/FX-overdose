...
namespace FXOverdose.P2P.Core
{
    ...
    public static class P2PNetworkTradingCodec
    {
        ...
        public static byte[] EncodeState(P2PTradeResult result, IReadOnlyList<P2PPlayerRuntimeState> ranked)
        {
            using var stream = new MemoryStream(); using var writer = new BinaryWriter(stream, Encoding.UTF8);
            writer.Write(result.RequestId); writer.Write((byte)result.RejectReason); writer.Write(result.FillPrice); writer.Write(ranked.Count);
        ...
        public static bool TryDecodeState(byte[] bytes, out P2PTradeResult result, out IReadOnlyList<P2PPlayerTradeSnapshot> players)
        {
            result = default; players = Array.Empty<P2PPlayerTradeSnapshot>();
            try { using var stream = new MemoryStream(bytes, false); using var reader = new BinaryReader(stream, Encoding.UTF8);
                result = new P2PTradeResult(reader.ReadUInt32(), (P2PTradeRejectReason)reader.ReadByte(), reader.ReadDouble());
