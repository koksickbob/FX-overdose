...
namespace FXOverdose.P2P.Infrastructure
{
    ...
    public sealed class NetworkCompetitionAuthority : MonoBehaviour
    {
        ...
        private void OnSteamClientDisconnected(ulong steamId){if(!net.IsServer||trading?.HostMatch==null)return;if(trading.HostMatch.ForfeitDisconnectedPlayer(steamId,(long)Time.unscaledTime)){Broadcast("연결이 끊긴 플레이어가 탈락했어용");trading.BroadcastCurrentState();}}
        ...
        private void OnDestroy(){var session=P2PNetworkSessionManager.Instance;if(session!=null)session.SteamClientDisconnected-=OnSteamClientDisconnected;if(!registered||net?.CustomMessagingManager==null)return;net.CustomMessagingManager.UnregisterNamedMessageHandler(ActionMessage);net.CustomMessagingManager.UnregisterNamedMessageHandler(StateMessage);}
    }
}
